...
## Step -1: Environment Verification (run autonomously, no checkpoint needed)
The user has hit repeated `ModuleNotFoundError` issues caused by `python3`
resolving to different interpreters across shell sessions/tabs
(`/opt/homebrew/.../python3.13` vs `/opt/anaconda3/.../python3.11`).
Before anything else:
1. Run `which python3` and `python3 -c "import sys; print(sys.executable)"`.
2. Run `python3 -c "import openai, daytona, pdfplumber, streamlit, fpdf"` —
   if any import fails, run
   `python3 -m pip install openai daytona pdfplumber streamlit fpdf2 --break-system-packages`
   using that SAME `python3`.
3. Then run, in order, fixing any errors yourself without waiting for
   user input (these are mechanical env/dependency issues, not design
   decisions):
   - `python3 generate_sample.py`
   - `python3 src/analyzer.py`
   - `python3 src/sandbox_validate.py`
   - `python3 src/terminal3_signer.py`
   - `streamlit run app.py` (start it, confirm it boots without
     import/syntax errors, then stop it -- don't leave it running)
4. ONLY after all four scripts run cleanly and Streamlit boots, report a
   short summary and move to Step 0 below (which DOES need checkpoints,
   per the Working Style rules in CLAUDE.md).

If any of these scripts fail for a reason OTHER than environment/imports
(e.g. an actual logic error, API auth failure, or a code bug), stop and
report that specific failure before proceeding -- that's a design issue,
